# Spec Delta: Slot Generation Engine
...
- [ ] TC-RF-CTR-AVAIL-010: Generate slots from weekly rule only (integration test)
- [ ] TC-RF-CTR-AVAIL-011: Exception overrides weekly rule (integration test)
- [ ] TC-RF-CTR-AVAIL-012: Blocks subtracted correctly (integration test)
- [ ] TC-RF-CTR-AVAIL-013: Confirmed bookings subtracted (integration test)
- [ ] TC-RF-CTR-AVAIL-019: Filter slots by service duration (integration test)
- [ ] TC-RF-CTR-AVAIL-017: Client can read slots publicly (integration test)
- [ ] TC-RNF-CTR-AVAIL-001: Performance P95 ≤ 800ms (k6 test)

---

## API Contracts

### GET /api/contractors/:contractorId/availability/slots

**Query Params**:
- `startDate` (required): YYYY-MM-DD
- `endDate` (required): YYYY-MM-DD (max 8 weeks from startDate)
- `serviceId` (optional): UUID (filter slots compatible with service duration)
...
**Errors**:
- 400: Invalid date range (endDate \u003c startDate, range \u003e 8 weeks, malformed dates)
...
async function generateSlots(
  ...
  // 1. Validate range
  if ((endDate - startDate).days \u003e 56) throw new Error("Max 8 weeks");

  // 2. Fetch contractor and timezone
  const contractor = await fetchContractor(contractorId);
  const timezone = contractor.location.timezone;

  // 3. Batch fetch availability data
  const weeklyRules = await fetchWeeklyRules(contractorId);
  const exceptions = await fetchExceptionsByDateRange(contractorId, startDate, endDate);
  const blocks = await fetchBlocksByDateRange(contractorId, startDate, endDate);
  const bookings = await fetchConfirmedBookings(contractorId, startDate, endDate);

  // 4. Optionally fetch service duration
  let minDuration = 0;
  if (serviceId) {
    const service = await fetchService(serviceId);
    minDuration = service.durationMinutes;
  }

  // 5. Generate slots per day
  ...
  for (const day of days) {
    ...
    // 5a. Get base intervals (exception overrides rule)
    let intervals: TimeInterval[] = [];
    const exception = exceptions.find(e =\u003e e.date === dateStr);

    if (exception) {
      if (exception.type === 'BLOCKED') continue; // Skip entire day
    ...
    if (intervals.length === 0) continue; // No availability for this day

    // 5b. Subtract blocks
    for (const block of blocks.filter(b =\u003e blockIntersectsDay(b, day))) {
    ...
    // 5c. Subtract confirmed bookings
    for (const booking of bookings.filter(b =\u003e bookingIntersectsDay(b, day))) {
    ...
    // 5d. Filter by service duration
    if (minDuration \u003e 0) {
      intervals = intervals.filter(int =\u003e calculateDurationMinutes(int) \u003e= minDuration);
    }
